...
object MyJournal extends App {
  ...
  def search() : Unit =  {
    ...
    println("Enter a field to search: ")
    println("Page - Search by page number")
    println("Date - Search by date")

    var command = scala.io.StdIn.readLine

    if(command.toUpperCase() == "PAGE") {

      println("Enter a condition (ex: < 6 or >= 5): ")
      var condition = scala.io.StdIn.readLine()

      var journals = JournalDao.searchByPage(condition)
